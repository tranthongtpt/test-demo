          ...
          {item.questions.map((question, index) => (
            <div key={question.id}>
              ...
              <div className='flex items-center py-2 my-2'>
                <p className='flex items-center justify-center w-6 h-6 p-1 mr-2 text-sm font-bold text-white bg-teal-600 rounded-full'>
                  {index + 1}
                </p>
                <p>
              ...
              {question.type === 'radio' && question.options && question.options.length > 0 && (
                <Select placeholder='Chọn đáp án' className='max-w-xs'>
                  {question.options.map((option, optionIndex) => (
                    <SelectItem key={optionIndex} className={`question_${question.id}`} value={option}>
                      {option}
                    </SelectItem>
                  ))}
                </Select>

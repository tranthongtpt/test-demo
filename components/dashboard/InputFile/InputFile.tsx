...
function InputFile({ setInputValues, setFileBefore, fileAfter, multi }: InputFileProps) {
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFiles) {
      ...
      if (invalidFiles.length > 0) {
        ...
        setMaxSize(false)
        setInputValues({ files: validFiles }) // Update the state with the selected files
        setFileBefore(validFiles) // Assuming setFileBefore accepts an array of Files directly
      }
    }
  }
  ...
  // }
  const handleFileRemove = (fileName: string) => {
    const updatedFiles = fileAfter.filter((file) => file.name !== fileName)
    setFileBefore(updatedFiles)
  }
